import { Alert, Box, HStack, Image, Stack, Text, VStack } from "@chakra-ui/react";
...
function CompDesc1(){
    ...
    const handleEnrollment = () => {
        if(!auth.currentUser){
            //display a alert box in the middle of the screen saying you need to login to enroll in the competition using chakra ui
             alert("You need to login to enroll in the competition");
         }else{
        ...
        axios.post(`https://codespace-iaeh.onrender.com/enroll-competition/${competitionId}/${userId}`,{name,email})
        .then((res) => {
            console.log(res);
            alert("You have been enrolled in the competition , now wait for the competition to start");
        })
        .catch((err) => {
            console.log(err);
            alert("You have already enrolled in the competition");
        })
    ...
    return(
        ...
        <Box>
            ...
            <HStack alignItems="top" w="1230px"  height="700px" ml={280} mt="15px" >
            
           <Link to={"/competition"}><Box w={30} h={30} borderRadius={15} bg={"#2ec866"} p={1.5} ml="-100px"><FaArrowLeft/></Box></Link>
                ...
                <Box h="695px" w="760px" borderWidth={2}>
                   ...
                   <Box  bg="#2EC866" w="120px" h="30px" paddingLeft={0} borderRadius={7} textAlign={"center"}>
                            <Text onClick={handleEnrollment} color="white" fontWeight={"bold"} paddingTop={1}>Register</Text>
                    </Box>
